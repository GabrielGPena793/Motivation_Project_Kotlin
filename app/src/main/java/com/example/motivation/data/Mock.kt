...
class Mock {
    ...
    private val all = MotivationConstants.Filter.INFINITY
    ...
    private val motivationPhrase = listOf(
    ...
    fun getPhrase(categoryId: Int, language: String): String {
        val phrases = motivationPhrase.filter {
            (it.categoryId == categoryId || categoryId == all)
                    && it.language == language
        }
        val maxLimit = phrases.size
        val randomNumber = Random.nextInt(maxLimit)

        return phrases[randomNumber].description
    }
